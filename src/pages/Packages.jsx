...
export default function Packages() {
  ...
  const loadPackages = async () => {
    try {
      const packagesData = await PackageService.getPublicPackages();
      // Check if data is paginated (DRF default)
                ...
                >
                  ...
                  <div className="mb-6">
                    <h3
                      className={`text-3xl font-black mb-2 ${
                        pkg.is_popular ? 'text-white' : 'text-gray-900'
                      }`}
                    >
                      {pkg.name}
                    </h3>
                    {pkg.description && (
                      <p
                        className={`text-lg ${
                          pkg.is_popular ? 'text-gray-300' : 'text-gray-600'
                        }`}
                      >
                        {pkg.description}
                      </p>
                      ...
                      {pkg.features.map((feature, i) => (
                        <div key={i} className="flex items-center gap-3">
                          ...
                          >
                            <Check
                              className={`w-4 h-4 ${
                                pkg.is_popular ? 'text-black' : 'text-white'
                              }`}
                            />
                          </div>
                          <span
                    ...
                    <div className="mb-6 pt-6 border-t border-gray-200">
                      <h4
                        className={`font-bold text-sm mb-3 ${
                          pkg.is_popular ? 'text-gray-300' : 'text-gray-700'
                        }`}
                      >
                        Available Add-ons:
                      </h4>
                      <div className="space-y-2">
                        {pkg.addons.slice(0, 3).map((addon, i) => (
                          <div
                            key={i}
                            className={`text-xs ${
                              pkg.is_popular ? 'text-gray-400' : 'text-gray-600'
                            }`}
                          >
                            + {addon.name} (€{addon.price}/mo)
                          </div>
                        ...
                        {pkg.addons.length > 3 && (
                          ...
                          >
                            + {pkg.addons.length - 3} more add-ons
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  <div
                    className={`mb-6 pt-6 border-t ${
                      pkg.is_popular ? 'border-yellow-400/30' : 'border-gray-200'
                    }`}
                  >
                    <div className="flex items-baseline gap-2">
                      <span
                        className={`text-4xl font-black ${
                          pkg.is_popular ? 'text-yellow-400' : 'text-gray-900'
                        }`}
                      >
                        €{pkg.price}
                      </span>
                      <span
                        className={`text-lg ${
                          pkg.is_popular ? 'text-gray-400' : 'text-gray-500'
                        }`}
                      >
                        /{pkg.duration}
                      </span>
