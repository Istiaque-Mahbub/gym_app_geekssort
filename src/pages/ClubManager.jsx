...
import { Link } from 'react-router-dom';
import { createPageUrl } from '@/utils';
import { Plus, Edit2, Trash2, ArrowLeft, Save, X, MoveUp, MoveDown, MapPin,Clock } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
...
export default function ClubManager() {
  ...
  const loadData = async () => {
    try {
      const response = await GymClubService.getClubAll();
      const data = Array.isArray(response.data) ? response.data : response.data.results;
      setClubs(data.sort((a, b) => (a.display_order || 0) - (b.display_order || 0)));
    } catch (error) {
  ...
  //   try {
  //     setUploading(true);
  //     // Replace with your real upload API if available
  ...
  // const handleWebsiteImageUpload = async (e) => {
  //   const file = e.target.files[0];
  //   if (!file) return;
  //   try {
  //     setUploading(true);
  //     const url = URL.createObjectURL(file); // Replace with real upload
  //     setFormData({ ...formData, homepage_image: url });
  //   } catch (error) {
  //     console.error('Error uploading website image:', error);
  //     alert('Error uploading website image');
  //   } finally { setUploading(false); }
  // };
  // Club main image
  ...
  const handleWebsiteImageUpload = (e) => {
    ...
    setFormData({ ...formData, homepage_image: file });
  };
  ...
  //     if (editingClub) {
  //       await GymClubService.update(editingClub.id, payload);
  //     } else {
  //       await GymClubService.create(payload);
  //     }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formDataToSend.append('weekdays_hours', formData.weekdays_hours);
      formDataToSend.append('weekend_hours', formData.weekend_hours);
      formDataToSend.append('display_order', formData.order);
      formDataToSend.append('show_on_homepage', formData.show_on_homepage);
      ...
      if (formData.homepage_image instanceof File) {
        formDataToSend.append('homepage_image', formData.homepage_image);
      }

      // Append facilities as JSON string
      const facilitiesData = formData.facilities
        .filter(f => f && f.trim() !== '')
        .map(f => ({ name: f.trim() }));

      formDataToSend.append('facilities', JSON.stringify(facilitiesData));

      if (editingClub) {
        await GymClubService.update(editingClub.id, formDataToSend);
      } else {
        await GymClubService.create(formDataToSend);
      }

      await loadData();
      resetForm();
    } catch (error) {
      console.error('Error saving club:', error.response?.data || error);
      alert('Error saving club');
    }
  };
  // --- Edit / Delete ---
const handleEdit = (club) => {
  ...
  // Map existing facilities to strings for inputs
  const facilitiesStrings = club.facilities?.map(f => f.name) || [''];
  ...
  setFormData({
    ...
    email: club.email || '',
    facilities: facilitiesStrings, // <-- now correctly as strings
    weekdays_hours: club.weekdays_hours || '',
    weekend_hours: club.weekend_hours || '',
    image: club.image || null,
    homepage_image: club.homepage_image || null,
    description: club.description || '',
    show_on_homepage: club.show_on_homepage || false,
    is_active: club.is_active ?? true,
    order: club.display_order || 0,
  });
  ...
  const handleDelete = async (id) => {
    if (!confirm('Are you sure you want to delete this club?')) return;
    try {
      await GymClubService.delete(id);
      await loadData();
    } catch (error) {
      console.error('Error deleting club:', error);
    }
  ...
  const removeFacility = (index) => { const f = [...formData.facilities]; f.splice(index, 1); setFormData({ ...formData, facilities: f }); };
  const updateFacility = (index, value) => { const f = [...formData.facilities]; f[index] = value; setFormData({ ...formData, facilities: f }); };
  ...
  const moveClub = async (index, direction) => {
    ...
    for (let i = 0; i < newClubs.length; i++) {
      await GymClubService.patch(newClubs[i].id, { display_order: i });
    }
    await loadData();
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="bg-black text-white py-8 px-6 shadow-lg">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <Button onClick={() => setShowForm(true)} className="bg-yellow-400 text-black hover:bg-yellow-500">
            <Plus className="w-5 h-5 mr-2" /> Add Club
          </Button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <h2 className="text-2xl font-black mb-6">{editingClub ? 'Edit Club' : 'Add New Club'}</h2>
            <form onSubmit={handleSubmit} className="space-y-6">
              {/* Name & Location */}
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Club Name *</label>
                  <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="e.g., FitHive Downtown" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2">Location *</label>
                  <Input value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} required placeholder="e.g., Downtown" />
                </div>
              </div>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Address *</label>
                <Input value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} required placeholder="Full address" />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Phone</label>
                  <Input value={formData.phone_number} onChange={(e) => setFormData({ ...formData, phone_number: e.target.value })} placeholder="+1 234 567 8900" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2">Email</label>
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Description</label>
                <Textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} rows={3} placeholder="Club description" />
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Weekdays Hours</label>
                  <Input value={formData.weekdays_hours} onChange={(e) => setFormData({ ...formData, weekdays_hours: e.target.value  })} placeholder="6:00 AM - 10:00 PM" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2">Weekend Hours</label>
                  <Input value={formData.weekend_hours} onChange={(e) => setFormData({ ...formData, weekend_hours: e.target.value })} placeholder="8:00 AM - 8:00 PM" />
                </div>
              </div>
              ...
              {/* <div className="grid md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold mb-2">Map Latitude</label>
                  <Input type="number" step="any" value={formData.map_lat} onChange={(e) => setFormData({ ...formData, map_lat: e.target.value })} placeholder="40.7128" />
                </div>
                <div>
                  <label className="block text-sm font-semibold mb-2">Map Longitude</label>
                  <Input type="number" step="any" value={formData.map_lng} onChange={(e) => setFormData({ ...formData, map_lng: e.target.value })} placeholder="-74.0060" />
                </div>
              </div> */}
              ...
              <div>
                ...
                {formData.facilities.map((facility, index) => (
                  <div key={index} className="flex gap-2 mb-2">
                    <Input value={facility} onChange={(e) => updateFacility(index, e.target.value)} placeholder="Facility name" />
                    <Button type="button" variant="outline" onClick={() => removeFacility(index)} disabled={formData.facilities.length === 1}><Trash2 className="w-4 h-4" /></Button>
                  </div>
                ))}
                <Button type="button" variant="outline" onClick={addFacility} className="mt-2"><Plus className="w-4 h-4 mr-2" /> Add Facility</Button>
              </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {/* {formData.images.map((img, idx) => ( */}
                    {/* <div className="relative">
                      <img src={formData.images} alt="" className="w-full h-24 object-cover rounded-lg" />
                      <Button type="button" size="icon" variant="destructive" className="absolute top-1 right-1 h-6 w-6" onClick={() => removeImage()}><X className="w-3 h-3" /></Button>
                    </div> */}
                  {/* ))} */}
                  {formData.image && (
                    <div className="relative">
                      <img
                        src={
                          formData.image instanceof File
                            ...
                            : formData.image // already uploaded URL
                        }
                        alt=""
              ...
              <div>
                <label className="block text-sm font-semibold mb-2">Website Homepage Image</label>
                <Input type="file" accept="image/*" onChange={handleWebsiteImageUpload} className="mb-3" />
                {formData.homepage_image && (
                  <div className="relative w-32">
                    <img
                      src={
                        formData.homepage_image instanceof File
                          ? URL.createObjectURL(formData.homepage_image)
                          : formData.homepage_image
                      }
                      alt=""
                      className="w-full h-40 object-cover rounded-lg"
                    ...
                    <Button
                      ...
                      variant="destructive"
                      className="absolute top-1 right-1 h-6 w-6"
                      onClick={() => setFormData({ ...formData, homepage_image: null })}
                    >
                      <X className="w-3 h-3" />
                    </Button>
                  </div>
              ...
              {/* Submit Buttons */}
              <div className="flex gap-3">
                <Button type="submit" className="bg-yellow-400 text-black hover:bg-yellow-500"><Save className="w-4 h-4 mr-2" /> {editingClub ? 'Update Club' : 'Create Club'}</Button>
        ...
        <div className="grid md:grid-cols-2 gap-6">
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <CardTitle>{club.name}</CardTitle>
                        {!club.is_active && <span className="bg-gray-300 text-gray-700 px-2 py-1 rounded text-xs font-bold">Inactive</span>}
                      </div>
                      <p className="text-sm text-gray-600 mt-1 flex items-center gap-1"><MapPin className="w-3 h-3" />{club.location}</p>
                    </div>
                    <div className="flex items-center gap-1">
                      {/* <Button size="icon" variant="outline" onClick={() => moveClub(index, 'up')} disabled={index === 0}><MoveUp className="w-4 h-4" /></Button>
                      <Button size="icon" variant="outline" onClick={() => moveClub(index, 'down')} disabled={index === clubs.length - 1}><MoveDown className="w-4 h-4" /></Button> */}
                      <Button size="icon" variant="outline" onClick={() => handleEdit(club)}><Edit2 className="w-4 h-4" /></Button>
                      <Button size="icon" variant="destructive" onClick={() => handleDelete(club.id)}><Trash2 className="w-4 h-4" /></Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  {/* {club.images?.length > 0 && <div className="grid grid-cols-3 gap-2 mb-4">{club.images.slice(0, 3).map((img, idx) => <img key={idx} src={img} alt="" className="w-full h-20 object-cover rounded-lg" />)}</div>} */}
                 <div className="grid grid-cols-3 gap-2 mb-4">{club.image && <img  src={club.image} alt="" className="w-full h-20 object-cover rounded-lg" />}</div>
                  <p className="text-sm text-gray-600 mb-3">{club.description}</p>
                  <div className="space-y-2 text-sm">
                    <p><span className="font-semibold">Address:</span> {club.address}</p>
                    {club.phone_number && <p><span className="font-semibold">Phone:</span> {club.phone_number}</p>}
                    {club.email && <p><span className="font-semibold">Email:</span> {club.email}</p>}
                    {club.operating_hours?.weekdays && <p><span className="font-semibold">Hours:</span> {club.operating_hours.weekdays}</p>}
                    ...
                    {club.facilities?.length > 0 && <div><p className="font-semibold">Facilities:</p><p className="text-gray-600">{club.facilities.slice(0, 3).map(f => f.name || f).join(', ')}{club.facilities.length > 3 && ` +${club.facilities.length - 3} more`}</p></div>}
                  </div>
                </CardContent>
              </Card>
            </motion.div>
          ...
          {clubs.length === 0 && <div className="col-span-2 text-center py-12"><p className="text-gray-500 text-lg">No clubs yet. Create your first club!</p></div>}
        </div>
      </div>
    </div>
  );
