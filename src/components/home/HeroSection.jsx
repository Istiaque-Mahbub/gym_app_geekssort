...
export default function HeroSection() {
  ...
  useEffect(() => {
    console.log('API URL:', slides);
    const loadBanners = async () => {
      try {
        const data = await BannerService.getAll();
        
        if (data?.data?.results?.length > 0) {
          const mappedSlides = data.data.results.map((item) => ({
            title: item.title2 || item.title1,
            subtitle: item.title1 || 'FITNESS',
            extra_title: item.title4 || '',
            media_type: item.background_type || 'image',
            desktop_url: item.background_file,
            cta_text: item.title3 || 'DISCOVER',
            cta_link: '/clubs',
          }));
    ...
    <section className="relative h-screen w-full overflow-hidden bg-black">
      <AnimatePresence mode="wait">
        ...
        >
          {slides[currentSlide].media_type === 'video' ? (
            ...
            <img
              src={slides[currentSlide].desktop_url}
              alt={slides[currentSlide].title}
              className="absolute inset-0 w-full h-full object-cover"
            />
          )}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 px-6">
        ...
        <motion.p
          ...
          className="text-white/70 text-lg md:text-xl mb-8 text-center"
        >
         {slides[currentSlide].extra_title}
          {/* FIND OUT MORE */}
        </motion.p>
